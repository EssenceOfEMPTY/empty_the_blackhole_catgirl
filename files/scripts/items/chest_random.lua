...
function make_random_card( x, y )
	...
	while ( valid == false ) do
		...
		if ( thisitem.spawn_requires_flag ) then
...
function drop_random_reward( x, y, chest, rand_x, rand_y, set_rnd_ )
	...
	while ( count > 0 ) do
		...
		elseif ( rnd <= 30 ) then
			...
			---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---
			local rem_timer = get_comp_value( GameGetWorldStateEntity( ), 'WorldStateComponent', nil, {
				{ 'perk_empty_gold_is_forever', false },
			}, nil )

			local amount, gold_url = 20, 'data/entities/items/pickup/goldnugget'
			rnd = Random( 0, 100 )
			...
			if ( rnd > 50 and rnd <= 70 ) then
				chest_load_gold_entity( gold_url .. '_50.xml', x + Random( -10, 10 ), y - 4 + Random( -10, 5 ), rem_timer )
			elseif ( rnd <= 80 ) then
				chest_load_gold_entity( gold_url .. '_200.xml', x + Random( -10, 10 ), y - 4 + Random( -10, 5 ), rem_timer )
			elseif ( rnd <= 90 ) then
				chest_load_gold_entity( gold_url .. '_1000.xml', x + Random( -10, 10 ), y - 4 + Random( -10, 5 ), rem_timer )
			else
			...
			for i=1, amount do
				chest_load_gold_entity( gold_url .. '.xml', x + Random( -10, 10 ), y - 4 + Random( -10, 5 ), rem_timer )
			end
		elseif ( rnd <= 40 ) then
			---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---
			-- Potion
